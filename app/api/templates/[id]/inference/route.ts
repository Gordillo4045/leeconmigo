...
const bodySchema = z.object({
  ...
  context_fragment: z.string().optional().default(""),
  correct_answer: z.enum(["verdadero", "falso", "indeterminado"]),
});
  ...
  try {
    ...
    const { data: row, error: insertErr } = await admin
      ...
      .insert({
        ...
        context_fragment: parsed.data.context_fragment ?? "",
    ...
    if (insertErr || !row) {
      return NextResponse.json(
        { error: "Error al crear afirmación", message: insertErr?.message },
        { status: 500 }
      );
    }
