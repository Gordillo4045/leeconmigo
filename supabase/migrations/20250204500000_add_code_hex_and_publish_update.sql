-- code_hex permite a la API buscar por código con el admin client (sin RPC ni auth).
alter table public.attempt_access_codes add column if not exists code_hex text;
update public.attempt_access_codes set code_hex = encode(code_hash, 'hex') where code_hex is null;
create index if not exists idx_attempt_access_codes_code_hex on public.attempt_access_codes(code_hex) where code_hex is not null;

-- publish_evaluation_session debe rellenar code_hex al insertar.
...
begin
  ...
  loop
    ...
    insert into public.attempt_access_codes (attempt_id, code_hash, code_hex, expires_at, created_by)
    values (v_attempt_id, v_code_hash, encode(v_code_hash, 'hex'), v_expires_at, v_actor);
